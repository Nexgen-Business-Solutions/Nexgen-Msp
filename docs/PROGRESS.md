...
| 2026-09-14 | agent principal | Revue d'interface (Playwright, comptes ZZTEST UI par rôle, 2FA réel) — corrigé : 46 runs de facturation laissés par les tests (runs soumis non supprimés) s'accrochaient à des noms de contrat réattribués (« Runs 3 », « couvert jusqu'au … » sur un contrat neuf) → runs purgés, `_sweep` supprime les runs soumis, et un run ne compte pour un contrat que si les deux ont le même customer ; message de refus neutre (ne nomme plus l'autre customer) ; pas de nouvelle tentative sur 403/404 (plus de spinner sans fin) ; portail : compteur « services in catalogue » = services du contrat, liste Services sans pied « No entries », liste de demandes vide « No request yet. ». Accès par rôle vérifiés : aucune page interdite atteinte, aucune donnée d'un autre customer affichée. |
| 2026-09-14 | agent principal | Revue d'interface, parcours complets (opérateur → accord du manager → technicien ; passations ; nouveau customer + contrat) — corrigé : une ligne « machine à préparer » d'une personne existante (`requested_for_user`, sans `client_user`) apparaissait sous « UNNAMED PERSON » dans la revue des lignes → `get_request` renvoie `requested_for_user` et le nom / département / username de cette personne, la revue groupe par `client_user`, puis `requested_for_user`, puis détenteur, et les faits de la personne la couvrent ; Prepare Device arrivait vide alors que le client avait saisi hostname / serial / type → la carte Device Provisioning reprend ces valeurs depuis les lignes qui attendent la même machine. Vérifié par l'interface : autorité (l'opérateur ne peut pas approuver, ses demandes attendent l'accord interne), accord du manager, passations Bob → Alice → stock avec historique du plus récent au plus ancien, création customer + contrat. |
| 2026-09-14 | agent principal | Revue de code et d'interface (suite) : les listes de départements des modales internes (New user, Edit, compte, droits) sont celles du customer de la personne (partagés + les siens, jamais ceux d'un autre customer) ; un service en Draft n'est plus compté comme en cours (tableau de bord Nexgen, KPI et liste appareils du portail, licences à récupérer) ; légende portail « X awaiting internal approval » = demandes en Awaiting Customer Approval ; la fiche personne Nexgen n'annonce plus « Already requested » pour une demande qui attend encore l'accord interne (le portail, lui, la voit toujours). Décision d'Idriss : les chiffres des KPI ne changent pas au clic, seule la liste est filtrée. |
| 2026-09-14 | agent principal | Demandes : un brouillon rouvre le formulaire (`/msp/requests/new?draft=…`) depuis la liste et depuis sa page ; une demande Completed / Rejected / Cancelled s'ouvre en récapitulatif sans stepper ni action (lignes par personne, « Open profile », ce qui a été exécuté). Brouillon rouvert par le personnel Nexgen : « Acting for which customer » reprend le customer de la demande, et l'étape Changes dit ce qui manque au lieu de rester vide. Mot de passe oublié (choix d'Idriss) : endpoint `request_password_reset` limité par adresse et par IP, « Account not found. » pour un compte inexistant ou désactivé. Idriss a renommé les étapes du formulaire (Users, Details) et masqué le bandeau Execution. |
| 2026-09-14 | agent principal | Traitement d'une demande (revue et exécution) : une personne à la fois, la liste des personnes à gauche avec ce qui reste pour chacune, Previous / Next en bas (`PeopleWorkspace`). Plus de doublon entre la ligne et le menu ⋯ : la ligne « Device » s'appelle « Assign a device » et ouvre la même modale que le menu (nouvelle machine ou machine existante, y compris une que la personne tient déjà) ; depuis le menu, si une ligne attend une machine, c'est cette ligne qui est réglée ; « Add service » du menu ne propose plus un service qu'une ligne de la demande ajoute déjà. Le personnel Nexgen n'a plus d'erreurs 400 sur le formulaire tant qu'il n'a pas choisi le customer. Portail : une demande refusée par le manager dit « Refused by your company » au lieu de « Nexgen's answer ». |
